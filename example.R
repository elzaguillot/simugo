...
## create a network
set.seed(32)
mockgeneset=makeset(nbgo=300,nbgenes=1000,nbgopergenes=50)
...
## test for enrichment
result=dotest(experiment,adjustPvalMethod="none") # for each gene set return the adjusted pvalue corresponding to different methods and the mean value of the bias of its genes
plot(result$pval_classic,col=result$active+1) # shows the pvalue color coded by the true activity of the gene set
abline(0.05,0,lty=2) # pvalue threshold at 0.05
